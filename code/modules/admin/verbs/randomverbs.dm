...
/client/proc/cmd_admin_create_centcom_report()
	...
	if(!holder)
		src << "Only administrators may use this command."
		return
	var/header = input(usr, "Enter the header for your report.")
	...
	if(confirm == "Yes")
		priority_announce(input, header, 'sound/AI/commandreport.ogg')
	else
		priority_announce("A report has been downloaded and printed out at all communications consoles.", "Incoming Classified Message", 'sound/AI/commandreport.ogg')

	print_command_report(input,"[confirm=="Yes" ? "" : "Classified "][command_name()] Update")

	log_admin("[key_name(src)] has created a command report: [header] [input]")
